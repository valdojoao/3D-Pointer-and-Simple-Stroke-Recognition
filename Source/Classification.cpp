...
void Classification::Classify( std::vector<double> DecPoints, const std::string& ClassifierStatus, const std::string& hand) {
...
//new function that I ve added to the classification class 
//receive a pair vector of x, y coordinates and pass to the classifier
void Classification::ClassifySample(std::vector<std::pair<double, double>> &handPoints) {

	for (int i = 0; i < handPoints.size(); i++) {             
               // Sample the classification points for prediction
               std::vector<double> v;
               v.push_back(handPoints[i].first);
               v.push_back(handPoints[i].second);
               Classification::Classify(v, "Sample", "");
             }
}